...
function migrate() {
    ...
    /*oldBlobService.getContainerProperties("messages", function(err, props){
       console.log("props=");
       console.log(props);
    });*/
...
function shortenArray(array, maxSize) {

    var newArray =[];
    for(i=0; i<maxSize && i<array.length; i++) {
        newArray[i] = array[i];
    }
    return newArray;
}

function getBlobs(marker) {
    ...
    oldBlobService.listBlobs("messages",options,function(error, blobs, continuation, response){
        ...
        if(!error){
            var shortened = shortenArray(blobs, 100);
             async.eachSeries(shortened,
                 ...
                 function(err) {
                    if(!err) {
                        console.log("COMPLETED!, going to next marker");
                        if(continuation.nextMarker) {
                            getBlobs(continuation.nextMarker);
                        }
                    }
                 }
             )
