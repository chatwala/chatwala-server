...
/**
 * Internal class for holding non-executed messagesAPI
 * @ignore
 */
...
var Base = function Base() {  
  ...
  // All messagesAPI not being executed
...
Base.prototype.__executeAllCallbacksWithError = function(err) {
  ...
  for(var j = 0; j < keys.length; j++) {
    ...
    if(info && info['chained'] && Array.isArray(info['chained']) && info['chained'].length > 0) {
      ...
      // Put back the final callback to ensure we don't call all messagesAPI in the chain
...
Base.prototype._registerHandler = function(db_command, raw, connection, exhaust, callback) {
  // If we have an array of messagesAPI, chain them
  ...
  if(chained) {
    ...
    // Register all the messagesAPI together
