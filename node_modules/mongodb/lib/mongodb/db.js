...
function Db(databaseName, serverConfig, options) {
  ...
  // Command queue, keeps a list of incoming messageAPI that need to be executed once the connection is up
  this.commands = [];
...
Db.prototype.open = function(callback) {
  ...
  if(self.serverConfig instanceof Server || self.serverConfig instanceof ReplSet || self.serverConfig instanceof Mongos) {
    ...
    self.serverConfig.connect(self, connect_options, function(err, result) {
      ...
      // If we have queued up messageAPI execute a command to trigger replays
...
/**
 * Execute a command hash against MongoDB. This lets you acess any messageAPI not available through the api on the server.
 *
 * Options
 *  - **maxTimeMS** {Number}, number of miliseconds to wait before aborting the query.
 *  - **ignoreCommandFilter** {Boolean}, overrides the default redirection of certain messageAPI to primary.
...
Db.prototype.command = function(selector, options, callback) {
  ...
  // Ensure only messageAPI who support read Prefrences are exeuted otherwise override and use Primary
...
var __executeInsertCommand = function(self, db_command, options, callback) {
  ...
  if(typeof callback === 'function') {
    ...
    if(errorOptions.w > 0 || errorOptions.w == 'majority' || errorOptions.j || errorOptions.journal || errorOptions.fsync) {      
      // db command is now an array of messageAPI (original command + lastError)
